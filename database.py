...
class DatabaseManager:
    ...
    def get_committees_count_by_month_and_hativa(self, start_date: date, end_date: date) -> List[Dict]:
        """Aggregate committees count per calendar month and division"""
        conn = self.get_connection()
        cursor = conn.cursor()

        cursor.execute('''
            SELECT strftime('%Y-%m', v.vaada_date) AS month,
                   h.name AS hativa_name,
                   h.color AS hativa_color,
                   COUNT(*) AS total_committees
            FROM vaadot v
            JOIN hativot h ON v.hativa_id = h.hativa_id
            WHERE v.vaada_date IS NOT NULL
              AND v.vaada_date >= ?
              AND v.vaada_date <= ?
              AND (v.is_deleted = 0 OR v.is_deleted IS NULL)
            GROUP BY month, hativa_name
            ORDER BY month ASC, hativa_name ASC
        ...
        return [{
            'month': row[0],
            'hativa_name': row[1],
            'hativa_color': row[2] or '#007bff',
            'total_committees': row[3]
        } for row in rows]

    def get_requests_by_maslul_per_month(self, start_date: date, end_date: date) -> List[Dict]:
        """Aggregate expected/actual requests per maslul per month"""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        cursor.execute('''
            SELECT strftime('%Y-%m', v.vaada_date) AS month,
                   m.name AS maslul_name,
                   h.name AS hativa_name,
                   SUM(COALESCE(e.expected_requests, 0)) AS expected_requests,
                   SUM(COALESCE(e.actual_submissions, 0)) AS actual_submissions
            FROM events e
            JOIN vaadot v ON e.vaadot_id = v.vaadot_id
            JOIN maslulim m ON e.maslul_id = m.maslul_id
            JOIN hativot h ON m.hativa_id = h.hativa_id
            ...
            GROUP BY month, maslul_name, hativa_name
            ORDER BY month ASC, expected_requests DESC
        ''', (start_date.isoformat(), end_date.isoformat()))
        ...
        return [{
            ...
            'maslul_name': row[1],
            'hativa_name': row[2],
            'expected_requests': row[3] or 0,
            'actual_submissions': row[4] or 0
        } for row in rows]

    def get_expected_requests_by_submission_month(self, start_date: date, end_date: date) -> List[Dict]:
        """Aggregate expected requests by submission month (call deadline)"""
        conn = self.get_connection()
        cursor = conn.cursor()

        cursor.execute('''
            SELECT strftime('%Y-%m', e.call_deadline_date) AS month,
                   SUM(COALESCE(e.expected_requests, 0)) AS expected_sum
            FROM events e
            WHERE e.call_deadline_date IS NOT NULL
              AND e.call_deadline_date >= ?
              AND e.call_deadline_date <= ?
              AND (e.is_deleted = 0 OR e.is_deleted IS NULL)
            GROUP BY month
            ORDER BY month ASC
