# מדריך למשתמש - מערכת מתכננת לניהול וועדות
...
מערכת מתכננת לניהול וועדות היא פלטפורמה ווב מתקדמת המיועדת לארגון ותזמון ישיבות ועדות, ניהול אירועים וארגון חטיבות במערכת מורכבת.
...
- **⏱️ חישוב מועדים אוטומטי** - כל המועדים מחושבים תוך התחשבות בימי עבודה והגדרות SLA המסלול
...
**כתובת המערכת:** https://izun.onrender.com/

· אין צורך בשם משתמש וסיסמה
...
#### 🏠 דף הבית
דף הבית הינו יומן אינטראקטיבי וזהו דף העבודה הראשי של המערכת – ממשק ויזואלי מתקדם המאפשר ניהול מהיר ויעיל
- **דשבורד מידע** - סטטיסטיקות כלליות של המערכת
- **תצוגה חודשית** - עם צבעי חטיבות
- **תפריטי פעולות חכמים** - לחיצה על תאריכים לפעולות מהירות
...
#### דרך דף הבית (מומלץ)
1. **מדף הבית**, לחץ על **תאריך עם ועדה קיימת**
...
#### דרך עמוד האירועים
1. **גש לעמוד "אירועים"**
2. **מדף הבית לחץ "הוסף אירוע" ובחר ועדה ומסלול**
3. **מלא את הפרטים:**
...
התזמון האוטומטי יוצר לוח זמנים חכם למספר חודש/ים בלחיצה עבור חטיבה/ות נבחרות, תוך כיבוד כל האילוצים העסקיים המתקדמים.
...
- **מגבלה שבועית**: מקסימום ישיבות בשבוע (ניתן להגדרה)
...
דף הבית הוא הלב של המערכת - ממשק ויזואלי מתקדם המאפשר ניהול מהיר ויעיל של ועדות ואירועים.
...
סל המחזור מאפשר שחזור של פריטים שנמחקו, תוך הפרדה בין מחיקה רכה למחיקה קבועה.

### מהי מחיקה רכה?

כאשר משתמש מוחק ועדה או אירוע, הפריט לא נמחק לגמרי מהמערכת אלא (לא נמחק לצמיתות עד לריקון סל המחזור):
- **מועבר לסל המחזור** - הפריט מסומן כ"מחוק" אך הנתונים נשמרים
- **מוסתר מהתצוגה הרגילה** - לא יופיע בדף הבית או ברשימות
...
- **Editor**: יכול לראות ולשחזר רק פריטים בחטיבות שלו
...
**ש: איך משתמש מתחיל להשתמש במערכת?**
ת: יש להיכנס ללינק https://izun.onrender.com/
...
ת: Viewer יכול רק לצפות בנתונים. Editor יכול גם ליצור ולערוך אירועים בחטיבות שלו. תקופת עריכה: ניתן לערוך כאשר תקופת העריכה פתוחה (בשליטת מנהל מערכת). Admin יכול הכל - עריכה מלאה.
