...
- **🎯 מערכת המלצות חכמה** - המלצות אוטומטיות לועדות מתאימות בעת הוספת אירוע
- **ניהול פרמטרי המלצות** - התאמה מלאה של מערכת הניקוד דרך דף האילוצים
...
│   ├── committee_recommendation_service.py  # 🆕 שירות המלצות
│   ├── constraints_service.py
│   └── audit_logger.py
├── templates/                # תבניות HTML
│   ├── base.html
│   ├── index.html            # 🆕 עם מערכת המלצות
│   ├── auto_schedule.html
│   ├── committee_types.html
│   ├── constraints.html      # 🆕 עם הגדרות המלצות
│   ├── admin/                # תבניות ניהול
...
4. **התחברות ראשונה**: השתמש במשתמש ברירת מחדל (admin/admin123)

## ניהול משתמשים

### משתמש Admin ברירת מחדל
בהפעלה ראשונה, המערכת יוצרת משתמש admin:
- **שם משתמש**: `admin`
- **סיסמה**: `admin123`
- **תפקיד**: מנהל מערכת
...
### תפקידי משתמשים
- **👑 Admin**: הרשאות מלאות, ניהול משתמשים, הגדרות מערכת
- **👨‍💼 Manager**: ניהול חטיבה ספציפית, יצירת ועדות ואירועים
- **👤 User**: צפייה ועריכה מוגבלת בהתאם להגדרות

### ניהול משתמשים (Admin בלבד)
1. **גישה**: תפריט משתמש → "ניהול משתמשים"
2. **יצירת משתמש**: כפתור "הוסף משתמש חדש"
3. **עריכת משתמש**: כפתור "ערוך" ליד כל משתמש
4. **שינוי סיסמה**: כפתור "סיסמה" ליד כל משתמש
5. **השבתת משתמש**: כפתור "השבת"/"הפעל"

### הרשאות מתקדמות
- **תקופות עריכה**: Admin יכול להפעיל/להשבית עריכה כללית
- **חטיבות ספציפיות**: Manager מוגבל לחטיבה שלו
- **אימות חטיבות**: המערכת מונעת פעולות חוצות חטיבות
...
### מערכת המלצות חכמה לועדות (אוקטובר 2025)
כעת בעת הוספת אירוע חדש, המערכת מציעה אוטומטית את הועדות המתאימות ביותר על סמך:
- זמינות מקום (מקסימום בקשות ביום)
- התאמה לדרישות SLA של המסלול
- עומס הועדה הקיים
- טווח זמן אופטימלי
- אילוצים שבועיים

**ניהול הגדרות**: כל פרמטרי ההמלצות ניתנים לכוונון דרך דף "ניהול אילוצים" (למנהלי מערכת)
